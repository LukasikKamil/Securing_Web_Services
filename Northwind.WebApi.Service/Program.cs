...
using Microsoft.AspNetCore.RateLimiting; // FixedWindowRateLimiterOptions
...
if (!useMicrosoftRateLimiting)
{
    ...
    builder.Services.Configure<ClientRateLimitOptions>(
        builder.Configuration.GetSection("ClientRateLimitLimiting"));
...
app.MapGet("api/customers/{id:int}",
    async Task<Results<Ok<Customer>, NotFound>> (
    [FromServices] NorthwindContext db,
    [FromRoute] string id) =>
        await db.Customers.FindAsync(id) is Customer customer ?
        TypedResults.Ok(customer) : TypedResults.NotFound())
...
app.MapGet("api/customers/{name}", (
    [FromServices] NorthwindContext db, [FromRoute] string name) =>
        db.Customers.Where(p => p.CompanyName.Contains(name)))
...
app.MapPut("/api/customers/{id}", async (string id, Customer customer, NorthwindContext db) =>
{
    var foundCustomer = await db.Customers.FindAsync(id);
